...
ct = ColumnTransformer(
    transformers=[
        ('imp',SimpleImputer(strategy='most_frequent'),['userid','state','gender','age','login_cnt_30','height','width']),
        ('imp2',SimpleImputer(strategy='constant',fill_value='UNK'),['sex','app_version'])
        # ('norm',Normalizer(),['state','login_cnt_30'])
    ])

ct.fit(df)
np.set_printoptions(suppress=True,   precision=10,  threshold=2000,  linewidth=5000)
res = ct.transform(df)
with open('t6.pickle','wb') as f:
    pickle.dump(ct,f)
print(df)
print('----------------------')
print(res)
...
print('===========================================')
ct = pickle.load(open('t6.pickle','rb'))
df2 = pd.DataFrame({'userid':[50],
                    'gender':[1],
                   'state':[1],
                    'height':[2340],
                   'age':[20],
