...
PAYNW_TAB['2015-12-12'] = PAYNW_TAB['2015-12-13']#修正双十二
...
# %%
TRN_N = 21#用来生成训练集的数据的天数
TST_N = 14#要预测的目标的天数，即用3周的数据来预测未来2周的销量
TST_PAD_N = 14 + 4
...
# %%
TRAIN = pd.DataFrame()
#('2015-06-26', '2015-07-16', '2015-07-17', '2015-07-30')
#('2015-06-27', '2015-07-17', '2015-07-18', '2015-07-31')
train_date_zip = zip(date_list[0:day_N - (TRN_N + TST_N) + 1], date_list[TRN_N - 1:day_N - TST_N + 1],
                     date_list[TRN_N:day_N - TST_N + 2], date_list[TRN_N + TST_N - 1:day_N])
...
PRECIP_TAB.columns = [str((datetime.datetime.strptime('20150501', '%Y%m%d') + datetime.timedelta(days=x)).date()) for x
                      in range(PRECIP_TAB.shape[1])]
...
WEATHER_TAB.columns = Const_Datestr3('RC_', '2015-06-26', '2016-11-20') + Const_Datestr3('RE_', '2015-06-26',
                                                                                         '2016-11-20') \
                      + Const_Datestr3('RG_', '2015-06-26', '2016-11-20') + Const_Datestr3('RI_', '2015-06-26',
                                                                                           '2016-11-20')
...
#
ALL_FEATURE_LIST = (
TRAIN_TRN_C + SHOP_columns + HOLI_TRN_CA + HOLI_TST_CA + HOLI_RATIO + PRECIP_TRN_C + PRECIP_TST_C + weather_output_columns)
