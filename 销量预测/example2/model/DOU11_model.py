...
SHOP_INFO_EN = pd.merge(SHOP_INFO_EN, category,
                        on = ['SHOP_CA1_EN','SHOP_CA2_EN','SHOP_CA3_EN'],how = 'left')

SHOP_INFO_EN['SHOP_CA1_EN'] = [(lambda x:(str(x.strip()) ) ) (x)
                               for x in SHOP_INFO_EN['SHOP_CA1_EN']]
PAYNW = pd.read_csv('../data_new/user_pay_new.csv')

PAYNW_TAB = pd.pivot_table(PAYNW, values=['Num_post'], index=['SHOP_ID'],
                           columns=['DATE'], aggfunc=np.sum)
PAYNW_TAB = pd.concat( [PAYNW_TAB[PAYNW_TAB.columns[0:169:1]],
                        pd.DataFrame({'A':[np.nan],},index=np.arange(1,2001)),
                        PAYNW_TAB[PAYNW_TAB.columns[169::1]] ], axis = 1)
PAYNW_TAB.columns = [str((datetime.datetime.strptime('20150626','%Y%m%d') + \
                          datetime.timedelta(days=x)).date())
                     for x in range( PAYNW_TAB.shape[1])]
...
tt2['ratio'] = tt2['2015-11-11'] / (
0.15 * tt2['2015-10-28'] + 0.35 * tt2['2015-11-04'] + 0.35 * tt2['2015-11-18'] + 0.15 * tt2['2015-11-25'])
...
tt2 = pd.merge(tt2[['SHOP_ID', 'SHOP_PAY', 'SHOP_SCO', 'SHOP_COM', 'SHOP_LEV', 'ratio'] + SHOP_SJ], SHOP_FEATURE,
               on=['SHOP_ID'], how='left')
SHOP_INFO_EN = pd.merge(SHOP_INFO_EN[['SHOP_ID', 'SHOP_PAY', 'SHOP_SCO', 'SHOP_COM', 'SHOP_LEV'] + SHOP_SJ],
                        SHOP_FEATURE, on=['SHOP_ID'], how='left')
