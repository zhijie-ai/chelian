...
def Const_Datestr(START_DATE, DATE_LEN):
    return[str((datetime.datetime.strptime(START_DATE,'%Y-%m-%d') +
                datetime.timedelta(days=x)).date()) for x in range( DATE_LEN)]


def Const_Datestr2(START_DATE, END_DATE):
    day_N = (datetime.datetime.strptime(END_DATE, '%Y-%m-%d') - datetime.datetime.strptime(START_DATE,
                                                                                           '%Y-%m-%d')).days + 1
...
def Const_Datestr3(Prefix, START_DATE, END_DATE):
    day_N = (datetime.datetime.strptime(END_DATE, '%Y-%m-%d') - datetime.datetime.strptime(START_DATE,
                                                                                           '%Y-%m-%d')).days + 1
    return_string = [
        (Prefix + str((datetime.datetime.strptime(START_DATE, '%Y-%m-%d') + datetime.timedelta(days=x)).date())) for x
        in range(day_N)]
