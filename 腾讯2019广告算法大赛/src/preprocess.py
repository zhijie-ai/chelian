...
def parse_rawdata():
    # 曝光日志
    df = pd.read_csv('data/testA/totalExposureLog.out', sep='\t',
    ...
    ##############################################################################
    # 静态广告
    df = pd.read_csv('data/testA/ad_static_feature.out', sep='\t',
                     names=['aid', 'create_timestamp', 'advertiser', 'good_id',
                            'good_type', 'ad_type_id','ad_size']).sort_values(by='create_timestamp')
    df = df.fillna(-1)
    ...
    ##############################################################################
    # 测试数据
    df = pd.read_csv('data/testA/test_sample.dat', sep='\t',
...
def construct_log():
    ...
    train_df['wday'] = wday# 天，
    train_df['hour'] = hour#小时
    train_df['minute'] = minute#分钟
    ...
    tmp.columns = ['aid', 'request_day', 'imp']# 每天每个广告的曝光次数
    log = log.merge(tmp, on=['aid', 'request_day'], how='left')
    log[log['request_day'] < 17973].to_pickle('data/user_log_dev.pkl')# 2019-02-17之前的数据
...
def construct_train_data(train_df):
    ...
    del train_df['bid']
    tmp_1.columns=['aid','request_day','bid']# 平均出价
    train_df=train_df.drop_duplicates(['aid','request_day'])
    train_df=train_df.merge(tmp,on=['aid','request_day'],how='left')# 加了一个曝光次数和bid_unique,平均出价
