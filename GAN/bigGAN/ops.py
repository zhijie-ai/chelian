...
def conv(x, channels, kernel=4, stride=2, pad=0, pad_type='zero', use_bias=True, sn=False, scope='conv_0'):
    ...
    with tf.variable_scope(scope):
        if pad > 0:
            ...
            if pad_type == 'zero' :
                print('BBBBBBBBBB',x.shape.as_list())
                x = tf.pad(x, [[0, 0], [pad_top, pad_bottom], [pad_left, pad_right], [0, 0]])
                print('CCCCCCCCCCCCC',x.shape.as_list())
            if pad_type == 'reflect' :
        ...
        if sn :
            ...
            print('EEEEEEEEEEEEEEEEEEEEEEEEE',x.shape.as_list())
            if use_bias :
...
def resblock(x_init, channels, use_bias=True, is_training=True, sn=False, scope='resblock'):
    print('X_init.shape',x_init.shape.as_list()) #[2048, 4, 4, 1536]
    with tf.variable_scope(scope):
        with tf.variable_scope('res1'): # pad > 0 & sn=False 只有填充功能，没有卷积
            x = conv(x_init, channels, kernel=3, stride=1, pad=1, use_bias=use_bias, sn=sn)
            x = batch_norm(x, is_training)
            x = relu(x)
            print('res1.X.shape', x_init.shape.as_list()) #[2048, 4, 4, 1536]

        with tf.variable_scope('res2'):
            x = conv(x, channels, kernel=3, stride=1, pad=1, use_bias=use_bias, sn=sn)
            x = batch_norm(x, is_training)
            print('res2.X.shape', x_init.shape.as_list()) #[2048, 4, 4, 1536]
...
def resblock_up_condition(x_init, z, channels, use_bias=True, is_training=True, sn=False, scope='resblock_up'):
    print('KKKKKKKKKKKKK,x_init.shape',x_init.shape.as_list(),channels)
    with tf.variable_scope(scope):
        with tf.variable_scope('res1'):
            x = condition_batch_norm(x_init, z, is_training)
            x = relu(x)
            x = deconv(x, channels, kernel=3, stride=2, use_bias=use_bias, sn=sn)
            print('LLLLLLLLLLLLLLLLL',x.shape.as_list())

        with tf.variable_scope('res2') :
            x = condition_batch_norm(x, z, is_training)
            x = relu(x)
            x = deconv(x, channels, kernel=3, stride=1, use_bias=use_bias, sn=sn)
            print('MMMMMMMMMMMMMMMM',x.shape.as_list())

        with tf.variable_scope('skip') :
            x_init = deconv(x_init, channels, kernel=3, stride=2, use_bias=use_bias, sn=sn)
            print('NNNNNNNNNNNNNNNNNNN',x_init.shape.as_list())
...
def self_attention_2(x, channels, sn=False, scope='self_attention'):
    with tf.variable_scope(scope):
        print('HHHHHHHHHHHHHHHHHHH',x.shape.as_list())
        f = conv(x, channels // 8, kernel=1, stride=1, sn=sn, scope='f_conv')  # [bs, h, w, c']
        print('FFFFFFFFFFFFFFFF',f.shape.as_list())
        f = max_pooling(f)
        print('GGGGGGGGGGGGGGGG',f.shape.as_list())

        g = conv(x, channels // 8, kernel=1, stride=1, sn=sn, scope='g_conv')  # [bs, h, w, c']
        print('IIIIIIIIIIIIIIII', g.shape.as_list())

        h = conv(x, channels // 2, kernel=1, stride=1, sn=sn, scope='h_conv')  # [bs, h, w, c]
        h = max_pooling(h)
        print('JJJJJJJJJJJJJJJJ',h.shape.as_list())#[2048, 32, 32, 48]

        # N = h * w
        s = tf.matmul(hw_flatten(g), hw_flatten(f), transpose_b=True)  # [bs, N, N][2048,4096,1024]

        beta = tf.nn.softmax(s)  # attention map

        o = tf.matmul(beta, hw_flatten(h))  # [bs, N, C] #[2048,4096,48]
        gamma = tf.get_variable("gamma", [1], initializer=tf.constant_initializer(0.0))
