...
#               不胜人生一场醉。                 #
#----------------------------------------------
# 该代码也是把q_loss增加到原始的G和D的loss中，再进行优化

import tensorflow as tf
import numpy as np
import logging
from infoGAN.impl4.mnist import Mnist
import tensorflow.contrib.slim as slim
from infoGAN.impl4.generator import generator
from infoGAN.impl4.discriminator import discriminator
from infoGAN.impl4.optimizer import optim
from infoGAN.impl4.queue_context import queue_context
import os
import sys
...
with tf.Session() as sess:
    ...
    try:
        while not coord.should_stop():
            ...
            sys.stdout.write(
                "process bar ::|" + "<" * dis_part + '|' + str(cur_step * 1.0 / num_batch_per_epoch * 100) + '%' + '\r')
            sys.stdout.flush()
            ...
            if cur_epoch > last_epoch:
                cur_step = 0
                print('cur epoch {0} update l_d step {1}, loss_disc {2}, loss_gen {3}'.format(cur_epoch, l_d_step, l_disc,
                                                                                        l_gen))
