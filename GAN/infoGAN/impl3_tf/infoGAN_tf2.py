...
#               不胜人生一场醉。                 #
#----------------------------------------------
# 参考代码:https://github.com/Y1ran/GAN-Network-Library-Tensorflow/blob/master/infoGAN.py

# 这个地址下有各种GAN的实现
# 该种形式是训练3个network，原始的D和G，新增的Q，Q网络的参数是D+G+Q
...
class infoGAN():
    ...
    def __init__(self,sess,epoch,batch_size,z_dim,dataset_name,
                 checkpoint_dir,result_dir,log_dir,SUPERVISED=True):
        self.sess = sess
    ...
    def build_model(self):
        ...
        D_fake,D_fake_logits,input4classifier_fake = self.discriminator(G,is_training=True,reuse=False)
        ...
        print('TTTTTTTTTTTTTT',q_disc_loss.shape)
        ...
        print('AAAAAAAAAAAAAA',len(t_vars),len(d_vars),len(g_vars),len(q_vars))
        print([var for var in t_vars if 'c_' in var.name])
        ...
        """" Testing """
        # for test
        self.fake_image = self.generator(self.z,self.y,is_training=False,reuse=False)
    ...
    def train(self):
        ...
        tf.global_variables_initializer().run()# 需要默认的session来运行
    ...
    def visualize_results(self, epoch):
        ...
        samples = self.sess.run(self.fake_image, feed_dict={self.z: z_sample, self.y: y_one_hot})
...
#
#         info = infoGAN(sess,50,32,100,'mnist','checkpoint','results','logs',True)
#         info.build_model()
#         info.train()
...
#下面这种方式之所以会出错，是因为代码中用到了tensor.run()执行该tensor，而tensor又不属于默认的session。
# sess = tf.Session()
# info = infoGAN(sess,50,32,100,'mnist','checkpoint','results','logs',True)
# info.build_model()
# info.train()
