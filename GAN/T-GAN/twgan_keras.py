...
class TGAN():
    ...
    def __str__(self):
        dit = self.__dict__
        show = ['l2_w', 'epochs', 'iters_per_sample', 'batch_size', 'lr', 'key_word']
    ...
    def train(self):
        ...
        plot_figure(D_loss, G_loss, 'png/{}_{}.png'.format(self.key_word, self.epochs))

    def d_loss_fn(self, x_real_fake_score, x_fake_real_score):
        loss = -K.mean(x_real_fake_score - x_fake_real_score)
        return loss

    def g_loss_fn(self, x_real_fake_score, x_fake_real_score):
        loss = -K.mean(x_fake_real_score - x_real_fake_score)
        return loss
