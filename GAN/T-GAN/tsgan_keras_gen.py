...
class TGAN():
    def __init__(self, args):
        ...
        # Build the generator
        self.generator = self.build_generator_()

    def __str__(self):
        dit = self.__dict__
        show = ['l2_w', 'epochs', 'iters_per_sample', 'batch_size', 'lr', 'key_word','g_lr', 'd_lr']
    ...
    def build_discriminator(self):
        # 编码器
        en_model = Sequential()
        en_model.add(Conv2D(img_dim, (5, 5), strides=(2, 2), padding='same',
                            input_shape=(img_dim,img_dim, 3),  kernel_regularizer=l2(self.l2_w)))
        en_model.add(LeakyReLU())
        for i in range(3):
            en_model.add(Conv2D(img_dim * 2 ** (i + 1), (5, 5), strides=(2, 2), padding='same',
                                kernel_regularizer=l2(self.l2_w)))
            en_model.add(BatchNormalization(momentum=self.momentum))
            en_model.add(LeakyReLU())
        en_model.add(GlobalAveragePooling2D())
        x_real = Input(shape=(img_dim, img_dim, 3))
        x_fake = Input(shape=(img_dim, img_dim, 3))
        x_real_encoded = en_model(x_real)
        x_fake_encoded = en_model(x_fake)
        ...
        dis_model.add(Dense(512, input_shape=(1024, ),  kernel_regularizer=l2(self.l2_w)))
        dis_model.add(LeakyReLU())
        dis_model.add(Dense(256,  kernel_regularizer=l2(self.l2_w)))
        dis_model.add(LeakyReLU())
        dis_model.add(Dense(128,  kernel_regularizer=l2(self.l2_w)))
        dis_model.add(LeakyReLU())
        dis_model.add(Dense(64,  kernel_regularizer=l2(self.l2_w)))
        dis_model.add(LeakyReLU())
        dis_model.add(Dense(32,  kernel_regularizer=l2(self.l2_w)))
        dis_model.add(LeakyReLU())
        dis_model.add(Dense(1, use_bias=False,  kernel_regularizer=l2(self.l2_w),
                  activation='sigmoid'))
        x_real_fake_score = dis_model(x_real_fake)
        x_fake_real_score = dis_model(x_fake_real)
        model = Model([x_real,x_fake],[x_real_fake_score,x_fake_real_score])
        return model
    ...
    def build_generator2(self):
        def build_generator(self):
            ...
            model.add(Dense(128 * 4 * 4, activation='relu', input_dim=z_dim))
    ...
    def build_generator(self):
        ...
        model.add(BatchNormalization(momentum=self.momentum))
        model.add(UpSampling2D())
        model.add(Conv2D(128, kernel_size=3, padding="same", kernel_regularizer=l2(self.l2_w)))
        model.add(Activation("relu"))
        model.add(BatchNormalization(momentum=self.momentum))
        model.add(UpSampling2D())
        model.add(Conv2D(64, kernel_size=3, padding="same", kernel_regularizer=l2(self.l2_w)))
        model.add(Activation("relu"))
        model.add(BatchNormalization(momentum=self.momentum))
        model.add(UpSampling2D())
        model.add(Conv2D(32, kernel_size=3, padding="same", kernel_regularizer=l2(self.l2_w)))
        model.add(Activation("relu"))
        model.add(BatchNormalization(momentum=self.momentum))
        model.add(UpSampling2D())
        model.add(Conv2D(16, kernel_size=3, padding="same", kernel_regularizer=l2(self.l2_w)))
        model.add(Activation("relu"))
        model.add(BatchNormalization(momentum=self.momentum))
        model.add(UpSampling2D())
        model.add(Conv2D(8, kernel_size=3, padding="same", kernel_regularizer=l2(self.l2_w)))
        model.add(Activation("relu"))
        model.add(BatchNormalization(momentum=self.momentum))
        model.add(Conv2D(3, kernel_size=3, padding='same', kernel_regularizer=l2(self.l2_w)))
        model.add(Activation("tanh"))
        return model
    ...
    def train(self):
        ...
        d_optimizer = tf.keras.optimizers.SGD(lr=self.d_lr, decay=1e-7, momentum=.9)
        g_optimizer = tf.keras.optimizers.SGD(lr=self.g_lr, decay=1e-7, momentum=.9)
        ...
        for i in tqdm(range(self.epochs)):
            ...
            with tf.GradientTape() as d:
                ...
                x_real_fake_score, x_fake_real_score = self.discriminator([x, x_gen])
                d_loss = self.d_loss_fn(x_real_fake_score, x_fake_real_score)
            ...
            with tf.GradientTape() as g:
                x_gen = self.generator(z)
                x_real_fake_score, x_fake_real_score = self.discriminator([x, x_gen])
                g_loss = self.g_loss_fn(x_real_fake_score, x_fake_real_score)
            ...
            print('------------------------------------------', 'x_real_fake_score',x_real_fake_score[:6],
                  'x_fake_real_score',x_fake_real_score[:6],
    ...
    def d_loss_fn(self, x_real_fake_score, x_fake_real_score):
        loss = -(K.mean(K.log(x_real_fake_score + 1e-9) + K.log(1 - x_fake_real_score)))
        return loss

    def g_loss_fn(self, x_real_fake_score, x_fake_real_score):
        loss = -(K.mean(K.log(1 - x_real_fake_score) + K.log(x_fake_real_score + 1e-9)))
        return loss
