...
#               不胜人生一场醉。                 #
#----------------------------------------------
from GAN.CycleGAN.impl3_tf.model import *
...
class CycleGAN():
    ...
    def input_setup(self):
        ...
        filenames_A = tf.train.match_filenames_once("../impl1_keras/datasetss/horse2zebra/trainA/*.jpg")
        self.queue_length_A = tf.size(filenames_A)
        filenames_B = tf.train.match_filenames_once("../impl1_keras/datasetss/horse2zebra/trainB/*.jpg")
        ...
        self.image_A = tf.subtract(
            tf.div(tf.image.resize_images(tf.image.decode_jpeg(image_file_A),
                                          [256, 256]), 127.5), 1)
        self.image_B = tf.subtract(
            tf.div(tf.image.resize_images(tf.image.decode_jpeg(image_file_B),
                                          [256, 256]), 127.5), 1)
    ...
    def model_setup(self):
        ...
        self.input_A = tf.placeholder(tf.float32,
                                      [batch_size, img_width, img_height, img_layer], name="input_A")
        self.input_B = tf.placeholder(tf.float32,
                                      [batch_size, img_width, img_height, img_layer], name="input_B")

        self.fake_pool_A = tf.placeholder(tf.float32,
                                          [None, img_width, img_height, img_layer],
                                          name="fake_pool_A")
