...
class ACGAN():
    ...
    def __init__(self,sess,epoch,batch_size,z_dim,dataset_name,
        ...
        if dataset_name=='mnist' or dataset_name=='fashion-mnist':
            ...
            # code
            self.len_discrete_code = 10 # categorical distribution
            self.len_continuous_code = 2 # gaussian distribution(e.g rotation,thickness)

            # load mnist
            self.data_X,self.data_y = load_mnist(self.dataset_name)

            # get number of batch for a single epoch
            self.num_batches = len(self.data_X//self.batch_size)
        else:
            raise NotImplementedError

    def classifier(self,x,is_training=True,reuse=tf.AUTO_REUSE):
        ...
        with tf.variable_scope('classifier',reuse=reuse):
            ...
            out_logits = linear(net,self.y_dim,scope='c_fc2')
    ...
    def build_model(self):
        ...
        D_real,D_real_logits,input4classifier_real = self.discriminator(self.inputs,
                                                                        is_training=True,reuse=False)

        # output of D for fake images
        G = self.generator(self.z,self.y,is_training=True,reuse=False)
        D_fake,D_fake_logits,input4classifier_fake = self.discriminator(G,
        ...
        code_real,code_logit_real = self.classifier(input4classifier_real,is_training=True)

        # For real samples
        q_real_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
            logits=code_logit_real,labels=self.y
        ))
        # For fake samples
        q_fake_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(
            logits=code_logit_fake,labels=self.y
        ))
...
if __name__ == '__main__':
    ...
    acgan = ACGAN(sess, 50, 32, 100, 'mnist', 'checkpoint', 'results', 'logs')
    acgan.build_model()
    acgan.train()
    sess.close()
