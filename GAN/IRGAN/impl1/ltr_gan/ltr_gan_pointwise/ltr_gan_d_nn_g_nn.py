...
# query_url_feature:存的是与query对应的url的feature,train+test=Large_norm
...
def generate_for_d(sess, model, filename):
    ...
    for query in query_pos_train:
        ...
        #算概率是针对当前query下所有的url来softmax的，也就是说，那个分布是由当前query下所有的url来计算的
...
def main():
    ...
    sess.close()
    param_best = pickle.load(open(GAN_MODEL_BEST_FILE))
