...
class ContextEncoder():
    ...
    def build_generator(self):
        ...
        plot_model(model,to_file='png/auto-encoder.png',show_shapes=True)

        masked_img = Input(shape=self.img_shape)
        gen_missing = model(masked_img)
        mod = Model(masked_img,gen_missing,'auto-encoder')
        plot_model(mod,show_shapes=True,to_file='png/generator.png')
        return mod
    ...
    def sample_images(self, epoch, imgs):
        ...
        for i in range(c):
            axs[0, i].imshow(imgs[i, :, :])#真实的图片
            axs[0, i].axis('off')
            axs[1, i].imshow(masked_imgs[i, :, :])#去掉部分图片的图片
            axs[1, i].axis('off')
            filled_in = imgs[i].copy()
            filled_in[y1[i]:y2[i], x1[i]:x2[i], :] = gen_missing[i]#生成的缺失部分
