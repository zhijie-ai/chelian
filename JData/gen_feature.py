...
action_1_path = "I:/Users/xiaojie/Desktop/资料/JData/data/JData_Action_201602.csv"
action_2_path = "I:/Users/xiaojie/Desktop/资料/JData/data/JData_Action_201603.csv"
action_3_path = "I:/Users/xiaojie/Desktop/资料/JData/data/JData_Action_201604.csv"
comment_path = "I:/Users/xiaojie/Desktop/资料/JData/data/JData_Comment.csv"
product_path = "I:/Users/xiaojie/Desktop/资料/JData/data/JData_Product.csv"
user_path = "I:/Users/xiaojie/Desktop/资料/JData/data/JData_User.csv"
...
def get_data_from_csv(path):
    reader = pd.read_csv(path,encoding='gbk',header=0,iterator=True)
    chunks = []
    loop = True
    while loop:
        try:
...
def get_actions_1():
    action = pd.read_csv(action_1_path,header=0,encoding='gbk',nrows=500000)
    # action = get_data_from_csv(action_1_path)
    return action

def get_actions_2():
    action2 = pd.read_csv(action_1_path,header=0,encoding='gbk',nrows=800000)
    # action2 = get_data_from_csv(action_2_path)
    return action2

def get_actions_3():
    action3 = pd.read_csv(action_1_path,header=0,encoding='gbk',nrows=700000)
    # action3 = get_data_from_csv(action_3_path)
    return action3
...
def get_action_feat(start_date, end_date):
    ...
    if os.path.exists(dump_path):
        ...
        print(actions[(actions.user_id==272629) & (actions.sku_id==107774)].head())#272629  107774
        print(actions.type.unique())
        actions = actions[['user_id', 'sku_id', 'type']]
        df = pd.get_dummies(actions['type'], prefix='%s-%s-action' % (start_date, end_date))
        actions = pd.concat([actions, df], axis=1)  # type: pd.DataFrame
        actions = actions.groupby(['user_id', 'sku_id'], as_index=False).sum()
        del actions['type']
        print('AAAAA',actions.columns)
        # pickle.dump(actions, open(dump_path, 'wb'))
    return actions

actions = get_action_feat('2016-02-01','2016-03-01')
print(actions.columns)
...
# get_accumulate_user_feat(train_start_date,train_end_date)
...
def make_train_set(train_start_date, train_end_date, test_start_date, test_end_date, days=30):
    ...
    if os.path.exists(dump_path):
        ...
        start_days = "2016-02-01"
        user = get_basic_user_feat()# 将年龄，性别，等级进行one-hot编码，返回[user_id,age,sex,lv]
        product = get_basic_product_feat()#返回sku_id,cate,brand,将a1，a2，a3进行one-hot编码
        user_acc = get_accumulate_user_feat(start_days, train_end_date) #从用户的角度返回累积特征，5个
        product_acc = get_accumulate_product_feat(start_days, train_end_date) #从产品的角度返回累积特征
        comment_acc = get_comments_product_feat(train_start_date, train_end_date) #将评论数one-hot编码，并返回
        labels = get_labels(test_start_date, test_end_date)#user_id,sku_id,label,label代表为1的记录
        ...
        for i in (1, 2, 3, 5, 7, 10, 15, 21, 30):
            ...
            if actions is None:
                actions = get_action_feat(start_days, train_end_date)
                print(actions.columns,len(actions.columns))
            else:
                actions = pd.merge(actions, get_action_feat(start_days, train_end_date), how='left',
                                   on=['user_id', 'sku_id'])
                print(actions.columns, len(actions.columns))
