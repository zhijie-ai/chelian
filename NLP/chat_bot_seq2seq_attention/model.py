...
#               不胜人生一场醉。                 #
#----------------------------------------------
# https://cloud.tencent.com/developer/article/1092249
# X只定长处理就够了，在训练阶段，decoder的输入处理，删除定长的序列中最后一个字符，在前面加'go'，
# target加eos再加pad定长处理,可以事先设定长度，也可以用当前batch的最长sequence为标准填充
# 在推理阶段，增加batch_size的数目的'go'开始，如果产生了eos，则结束，在推理阶段，是没有y值的。
# 而本代码在处理target数据时，只做了定长处理，并没有加eos
# 在train的时候，加了go
# 在推理的时候，只是指明了以go开始，eos结束，也没有加到数据中，其实，推理的时候，根本加不了，因为没有target

# dynamic_rnn中的inputs，每个batch数据必须相同，不同batch之间可以不用相同。
# https://cloud.tencent.com/developer/article/1016695
# static_rnn中，每个batch必须填充到max_sequence的长度
