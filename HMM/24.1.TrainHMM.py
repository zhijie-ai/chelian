...
def calc_alpha(pi, A, B, o, alpha):
    for i in range(4):
        alpha[0][i] = pi[i] + B[i][ord(o[0])]  # 本来相乘，但是取了log就变成相加了
    T = len(o)
...
def calc_ksi(alpha, beta, A, B, o, ksi):
    ...
    for t in range(T - 1):
        ...
        for i in range(4):
            for j in range(4):
                ksi[t][i][j] = alpha[t][i] + A[i][j] + B[j][ord(o[t + 1])] + beta[t + 1]
                temp[k] = ksi[t][i][j]
                k += 1
        s = log_sum(temp)
        for i in range(4):
...
def baum_welch(pi, A, B):
    ...
    for time in range(3):
        print('calc_alpha')
        # alpha(t,i):给定lamda，在时刻t的状态为i且观测到o(1),o(2)...o(t)的概率
        calc_alpha(pi, A, B, sentence, alpha)
        # beta(t,i)：给定lamda和时刻t的状态i，观测到o(t+1),o(t+2)...oT的概率
        print('calc_beta')
        calc_beta(pi, A, B, sentence, beta)
        # gamma(t,i)：给定lamda和O，在时刻t状态位于i的概率
        print('calc_gamma')
        calc_gamma(alpha, beta, gamma)
        # ksi(t,i,j)：给定lamda和O，在时刻t状态位于i且在时刻i+1，状态位于j的概率
        print('calc_ksi')
        calc_ksi(alpha, beta, A, B, sentence, ksi)
        print('bw')
...
def mle():  # 0B/1M/2E/3S
    ...
    b = [[0] * 65536 for x in range(4)]  # nb[i][o]:从状态i到字符o的个数
