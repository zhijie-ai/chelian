...
class Model():
    ...
    def __init__(self,args,data,infer = False):
        ...
        with tf.name_scope('model'):
            ...
            self.initial_state = self.cell.zero_state(args.batch_size,tf.int32)
            ...
            outputs,last_state = tf.nn.dynamic_rnn(self.cell,inputs,
                                                    initial_state=self.initial_state)
            with tf.name_scope('loss'):
                output  = tf.reshape(outputs,[-1,args.state_size])
                self.logis = tf.matmul(output,w) + b
                self.probs = tf.nn.softmax(self.logis)
                self.last_state = last_state
                targets = tf.reshape(self.target_data,[-1])
                loss = seq2seq.sequence_loss_by_example([self.logis],[targets],
            ...
            with tf.name_scope('optimize'):
                ...
                # self.merged_op = tf.merged_all_summaries()
...
if __name__ == '__main__':
    ...
    Usage:
    Training:
        python3 LyricsGenerator.py 0
    Sampling:
        python3 LyricsGenerator.py 1
    """
