...
class Model(object):
    ...
    def __init__(self, user_count, item_count, cate_count, cate_list, predict_batch_size, predict_ads_num):
        ...
        self.i = tf.placeholder(tf.int32, [None,]) # [B]相当于target的sku,y为1代表正例的sku，为0代表负例的sku
...
def attention(queries, keys, keys_length):
    '''
      首先将query和key变成维度一样，然后操作并拼接成一个向量送入ff网络中。
      queries:     [B, H]
      keys:        [B, T, H]
      keys_length: [B]
    '''
...
def attention_multi_items(queries, keys, keys_length):
    ...
    key_masks = tf.tile(key_masks, [1, queries_nums])#[B, T*N]
    ...
    keys = tf.reshape(keys, [-1, max_len, queries_hidden_units])#[B*N, T, H]   [B, N, T, H]
