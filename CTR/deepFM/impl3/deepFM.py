...
class DeepFM:
    ...
    def _qcut(self, x, q, labels=None, retbins=False, precision=3):
        ...
        return pd.qcut(x,bins,labels=labels,precision=precision)

    def sigmoid(self, logits):
        return (1 / (1 + tf.exp(-logits)))

    def load_data(self, train_dfn="data/adult.data", test_dfn="data/adult.data"):
        '''
    ...
    def prepare_input_data(self, input_data):
        ...
        td = td[self.continuous_columns + list(self.categorical_columns.keys())]
        ...
        for col in self.continuous_columns:
            ...
            bins = self._qcut(td[col].values, bins_count, labels=False, retbins=True)
    ...
    # X 为连续特征+离散one-hot之后的特征，X_code为原始特征数目，每一列代表该特征下的labelencoder
    def build_model(self, X, X_code):

        sample_size, feature_size = X.get_shape().as_list()
        feature_code_size = X_code.get_shape().as_list()[1]#原始特征个数
        print('AAAA',X.get_shape(),X_code.get_shape())#AAAA (1000, 114) (1000, 9)
        ...
        # interaction factors, randomly initialized，这里应该是feature_size*embed_size
        random_vector = np.random.rand(sample_size, embed_size).astype(np.float32)
        inter_vector = tf.Variable(random_vector)#N*K的矩阵
        ...
        for i in range(feature_code_size):
            for j in range(1, feature_code_size - i):
                ...
                second_order = second_order + tf.reshape(tf.reduce_sum(tf.multiply(former, later),
                                                                       1, keep_dims=True),
                                                         [sample_size, 1])
    ...
    def train(self, n_epoch=1000, snapshot_step=10, batch_size=1000, learning_rate=0.01):
        ...
        # X_codes_mat为类别特征加数值特征并将相应的特征值变为0,1,2,3,4这样的数字(离散特征+连续特征)
        # X_mat为原来的连续特征+离散特征one-hot之后的特征
        # X_mat onehot之后的特征，X_codes_mat:每个特征下存的是0,1，2,3,4，这样的特征。
        ...
        sample_size, feature_size = X_mat.shape#[batch_size,p]#连续特征+离散特征one-hot
        feature_code_size = X_codes_mat.shape[1]#原始特征个数
