...
# 由于可以化简，所以并不需要根据index获取对应的vector的操作。想FFM中，需要根据index获取
#     vector操作
# https://github.com/zhijie-ai/CTR_Prediction/blob/master/FM/FM.py
# 形式化的表示就是建模P(click|content)给定上下文，建模点击/购买/下载/使用的概率。
...
def train_model(sess, model, epochs=10, print_every=50):
    ...
    with open('data/train_sparse_data_frac_0.01.pkl', 'rb') as f:
    ...
    for e in range(epochs):
        ...
        for ibatch in range(num_batches):
            ...
            feed_dict = {model.X: (batch_indexes, batch_values, batch_shape),#还能传入指定(index,column,values)的形式？
                         model.y: batch_y,
                         model.keep_prob:1.0}
...
if __name__ == '__main__':
    ...
    train_array_length = max(fields_dict.values()) + 1#fields_dict中有多少个key，说明就有多少个feature
