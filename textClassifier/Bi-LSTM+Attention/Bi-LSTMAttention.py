...
class BiLSTMAttention():
    ...
    def __init__(self, config, wordEmbedding):
        ...
        print('AAAAAAAAAAA',self.embeddedWords.shape)
        # 将最后一层Bi-LSTM输出的结果分割成前向和后向的输出
        outputs = tf.split(self.embeddedWords, 2, -1)
        print('BBBBBBBBBBB', outputs[0].shape,outputs[1].shape)
    ...
    def attention(self,H):
        ...
        print('tf.transpose(H, [0, 2, 1]).shape',tf.transpose(H, [0, 2, 1]).shape)
