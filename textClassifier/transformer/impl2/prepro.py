...
def prepro(hp):
    ...
    test1 = "iwslt2016/de-en/IWSLT16.TED.tst2013.de-en.en.xml"
    test2 = "iwslt2016/de-en/IWSLT16.TED.tst2013.de-en.de.xml"
    ...
    # train
    _prepro = lambda x:  [line.strip() for line in open(x, 'r',encoding='utf8').read().split("\n") \
    ...
    _prepro = lambda x: [re.sub("<[^>]+>", "", line).strip() \
                     for line in open(x, 'r',encoding='utf8').read().split("\n") \
    ...
    logging.info("prepro_test1:{}".format(prepro_test1[0]))
    logging.info("prepro_test2:{}".format(prepro_test2[0]))
