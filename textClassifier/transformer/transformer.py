...
#               不胜人生一场醉。                 #
# ----------------------------------------------
# https://github.com/jiangxinyang227/textClassifier/blob/master/Transformer/transformer.ipynb
import os
import csv
import time
import datetime
...
class Transformer(object):
    ...
    def __init__(self, config, wordEmbedding):
        # 定义模型的输入
        self.inputX = tf.placeholder(tf.int32, [config.batchSize, config.sequenceLength], name="inputX")
        self.inputY = tf.placeholder(tf.int32, [config.batchSize], name="inputY")

        self.dropoutKeepProb = tf.placeholder(tf.float32, name="dropoutKeepProb")
        self.embeddedPosition = tf.placeholder(tf.float32,
                                               [config.batchSize, config.sequenceLength, config.sequenceLength],
    ...
    def _multiheadAttention(self, rawKeys, queries, keys, numUnits=None, causality=False, scope="multiheadAttention"):
        ...
        print('Q.shape', Q.shape)  # (128, 200, 400)
        ...
        print('Q_.shape', Q_.shape)  # 1024, 200, 50)
        print('queries.shape', queries.shape)  # (128, 200, 400)

        # 计算keys和queries之间的点积，维度[batch_size * numHeads, queries_len, key_len], 后两维是queries和keys的序列长度
        similary = tf.matmul(Q_, tf.transpose(K_, [0, 2, 1]))
        print('similary.shape', similary.shape)  # (1024, 200, 200)
        ...
        print('rawKeys.shape', rawKeys.shape)  # (128, 200)
        print('keyMasks.shape', keyMasks.shape)  # (1024, 200)
        print('tf.expand_dims(keyMasks, 1).shape', tf.expand_dims(keyMasks, 1).shape)  # (1024, 1, 200)
        ...
        print('paddings.shape', paddings.shape)  # (1024, 200, 200)
