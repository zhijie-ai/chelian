...
class Dataset(object):
    ...
    def _genVocabulary(self, reviews):
        ...
        #vocab 为在alphabet的基础上加了UNK,pad2个字符的的字符集，
        # charEmbedding为alphabet+UNK的onehot编码矩阵
...
class CharCNN():
    ...
    def __init__(self,config,charEmbedding):
        ...
        with tf.name_scope("embedding"):
            ...
            print('BBBBBBBBBB',self.embededChars.shape)

        for i, cl in enumerate(config.model.convLayers):
            print('CCCCCCCCCCC', self.embededCharsExpand.shape)
            ...
            with tf.name_scope('convLayer-%s'%(i+1)):
                ...
                conv = tf.nn.conv2d(self.embededCharsExpand, wConv,
                                    strides=[1, 1, 1, 1], padding="VALID", name="conv")
                # 加上偏差
                hConv = tf.nn.bias_add(conv, bConv)
                # 可以直接加上relu函数，因为tf.nn.conv2d事实上是做了一个卷积运算，
                # 然后在这个运算结果上加上偏差，再导入到relu函数中
                ...
                print('AAAAAAAAAAAA',hPool.shape)
        ...
        with tf.name_scope("outputLayer"):
            ...
            wOut = tf.Variable(tf.random_uniform([config.model.fcLayers[-1], 1],
            ...
            # 进行二元分类
            self.binaryPreds = tf.cast(tf.greater_equal(self.predictions, 0.0),
                                       tf.float32, name="binaryPreds")

        with tf.name_scope("loss"):
            # 定义损失函数，对预测值进行softmax，再求交叉熵。

            losses = tf.nn.sigmoid_cross_entropy_with_logits(logits=self.predictions,
                                                             labels=self.inputY)
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        # 保存模型的一种方式，保存为pb文件
        savedModelPath = "../model/charCNN/savedModel"
