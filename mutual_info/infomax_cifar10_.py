...
print(x_train.shape,y_train.shape,x_test.shape,y_test.shape,img_dim)
...
feature_map = x # 截断到这里，认为到这里就是feature_map(局部特征)
...
x = GlobalAveragePooling2D()(x)#全局特征
print('BBBBBBBBB',x.shape)
...
prior_kl_loss = -0.5*K.sum(1+z_log_var-K.square(z_mean)-K.exp(z_log_var),axis=-1)
prior_kl_loss = K.mean(prior_kl_loss)
print('TTTTTTTTTTTT',z_log_var.shape)
...
print('z_z_2.shape',z_z_2.shape)

# 与随机采样的特征拼接(局部)
feature_map_shuffle = Lambda(shuffling)(feature_map)
print('feature_map_shuffle.shape',feature_map_shuffle.shape)
z_samples_repeat = RepeatVector(16)(z_samples)
print('z_samples_repeat.shape',z_samples_repeat.shape)
z_samples_map = Reshape((4,4,z_dim))(z_samples_repeat)#(?, 4, 4, 256)
z_f_1 = Concatenate()([z_samples_map,feature_map])
z_f_2 = Concatenate()([z_samples_map,feature_map_shuffle])# (?, 4, 4, 512)
print('CCCCCCCCCCCC',z_samples_map.shape,feature_map.shape,feature_map_shuffle.shape)
...
print('GlobalDiscriminator.shape',z.shape)

GlobalDiscriminator = Model(z_in,z)
z_z_1_score = GlobalDiscriminator(z_z_1)
z_z_2_score = GlobalDiscriminator(z_z_2)
print('z_z_2_score.shape',z_z_2_score.shape)
global_info_loss = -K.mean(K.log(z_z_1_score+1e-6)+K.log(1-z_z_2_score+1e-6))
print('global_info_loss:',global_info_loss)
...
local_info_loss = -K.sum(K.log(z_f_1_score+1e-6)+K.log(1-z_f_2_score+1e-6),
...
model_train.fit(x_train,epochs=50,batch_size=64,verbose=True)
...
def sample_knn(path):
    ...
    misc.imsave(path+'_l2.png',figure1)
    misc.imsave(path+'_cos.png',figure2)
