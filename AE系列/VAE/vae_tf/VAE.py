...
#               不胜人生一场醉。                 #
#----------------------------------------------
# https://github.com/Y1ran/GAN-Network-Library-Tensorflow/blob/master/VAE.py

import time

import AE系列.VAE.vae_tf.prior_factory as prior
from AE系列.VAE.vae_tf.ops import *
...
class VAE():
    ...
    def encoder(self,x,is_training = True,reuse = False):
        ...
        with tf.variable_scope('encoder',reuse=reuse):
            ...
            net = lrelu(bn(conv2d(net,128,4,4,2,2,name='en_conv2'),is_training=is_training,
                           scope='en_bn2'))
            ...
            # The standard deviation must be positive .Parametrize with a softplus and add
    ...
    def build_model(self):
        ...
        KL_divergence = 0.5*tf.reduce_sum(
            tf.square(self.mu)+tf.square(sigma)-tf.log(1e-8+tf.square(sigma))-1,[1])
        ...
        with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
            self.optim = tf.train.AdamOptimizer(self.learning_rate*5,
...
if __name__ == '__main__':
    ...
    info = VAE(sess, 50, 32, 100, 'mnist', 'checkpoint', 'results', 'logs')
    info.build_model()
    info.train()
    sess.close()